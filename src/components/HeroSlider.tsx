      ...
      {newsData.map((news, index) => (
        ...
        >
          ...
          <div className="relative z-20 flex flex-col justify-center h-full px-6 md:px-16 w-full max-w-7xl mx-auto">
            <div 
              className={`max-w-2xl transition-all duration-700 ease-out bg-slate-900/40 backdrop-blur-sm p-6 md:p-8 rounded-3xl border border-white/10 ${
          ...
          {newsData.map((_, index) => (
            <button 
              ...
              className={`h-2 rounded-full transition-all duration-500 overflow-hidden ${
                index === current ? "bg-primary-green w-16 md:w-24" : "bg-white/30 hover:bg-white/50 w-6"
              }`}
