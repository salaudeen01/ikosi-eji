...
//   const category  = "";
  ...
  const categoryArticles: Record<string, any[]> = {
    "market-news": [
      ...
      {
        image: "https://res.cloudinary.com/orestech/image/upload/v1759767961/tech-startup_gr3slq.jpg",
        category: "Market News",
  ...
  const articles = categoryArticles[category?.category || ""] || [];
  const title = categoryTitles[category?.category || ""] || "Articles";
  const heroData = categoryHeroData[category?.category || ""];
        ...
        <div className="container mx-auto px-4 py-12">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {articles.map((article, index) => (
              <ArticleCard key={index} {...article} category={category?.category} />
            ))}
          </div>
        </div>
      </div>
